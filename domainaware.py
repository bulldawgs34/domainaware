...
def twist_query(dnstwist_path, domain):
    """Query dnstwist"""
    rows = []
    dt_args = [dnstwist_path, '-gr', '-f', 'csv', domain] #edited out csv specification, modified dnstwist script to default to csv output
...
def crazy_query(urlcrazy_path, domain, attempt=1):
    ...
    except (UnicodeDecodeError, ProcessLookupError, ChildProcessError) as e:
        attempt += 1
        if attempt > max_attempts:
            print("Warning: Failed to parse URLCrazy output for {}".format(domain, e), file=stderr)
            return []
...
def main():
    ...
    slack_config = dict(config.items("slack"))
